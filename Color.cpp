...
bool Color::InitializeWindow(HINSTANCE hInstance,
    int ShowWnd,
    bool fullscreen)
...
bool Color::InitD3D()
{
    ...
    D3D12_HEAP_PROPERTIES heapsProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    D3D12_RESOURCE_DESC   resourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(vBufferSize);
    ...
    device->CreateCommittedResource(
        &heapsProperties, // a default heap
        D3D12_HEAP_FLAG_NONE, // no flags
        &resourcesDesc, // resource description for a buffer
   ...
   heapsProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
   resourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(vBufferSize);

    device->CreateCommittedResource(
        &heapsProperties, // upload heap
        D3D12_HEAP_FLAG_NONE, // no flags
        &resourcesDesc, // resource description for a buffer
    ...
    D3D12_RESOURCE_BARRIER barriers = CD3DX12_RESOURCE_BARRIER::Transition(vertexBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
   ...
   heapsProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
   resourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(iBufferSize);
   ...
   resourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(vBufferSize);
    ...
    // transition the vertex buffer data from copy destination state to vertex buffer state
    barriers = CD3DX12_RESOURCE_BARRIER::Transition(indexBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
    commandList->ResourceBarrier(1, &barriers);
    ...
    heapsProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT);
    resourcesDesc = CD3DX12_RESOURCE_DESC::Tex2D(DXGI_FORMAT_D32_FLOAT, Width, Height, 1, 0, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
    ...
    for (int i = 0; i < frameBufferCount; ++i)
    {
        ...
        D3D12_HEAP_PROPERTIES heapsProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        D3D12_RESOURCE_DESC   resourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(1024 * 64);

        hr = device->CreateCommittedResource(
            &heapsProperties, // this heap will be used to upload the constant buffer data
            D3D12_HEAP_FLAG_NONE, // no flags
            &resourcesDesc, // size of the resource heap. Must be a multiple of 64KB for single-textures and constant buffers
...
void Color::WaitForPreviousFrame()
{
    HRESULT hr;
